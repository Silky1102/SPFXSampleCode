...
interface IProps {
  ...
  onEdit: (doc: IDocument) => void;
    onEmail: (doc: IDocument) => void;
  onDelete: (doc: IDocument) => void;
}


    
const EmployeeDocumentList : React.FC<IProps> = ({ documents, onEdit, onDelete,onEmail }) => {
  ...
  const [hideDialog, setHideDialog] = React.useState(true);
    ...
    { key: 'Department', name: 'Department', fieldName: 'Department',minWidth: 50, maxWidth: 50},
    {
    ...
    },
    {
      ...
      onRender: item => <IconButton iconProps={{ iconName: 'Delete' }} onClick={() => showDeleteDialog(item)} />
        ...
        {
      key: 'email', name: 'Send Email', fieldName: 'email', minWidth: 100, maxWidth: 200,
      onRender: item => <IconButton iconProps={{ iconName: 'Mail' }} onClick={() => onEmail(item)} />
    }
  ];
  ...
  <div>
    ...
    <DetailsList items={documents} columns={columns} selectionMode={SelectionMode.single} compact={true}/>
